...
const membersData = [
  ...
  { id: 2, name: 'NWAEFULU ANNA EZIAFAKEGO', position: 'VICE PRESIDENT', image: '/images/VICE PRESIDENT.jpg' },
    ...
    <div>
      <Slider {...settings}>
        {membersData.map((member) => (
          <div key={member.id}>
            <Image src={member.image} alt={member.name} width={339} height={339} className='w-full h-auto rounded-2xl memberImg' />
            <h5>{member.name}</h5>
            <p>{member.position}</p>
          </div>
        ))}
