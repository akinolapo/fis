...
const membersData = [
  ...
  { id: 3, name: 'OZONECHI COLLINS IKENNA
  SECRETARY', position: 'Designer', image: '/alice.jpg' },
    ...
    <div>
      <Slider {...settings}>
        {membersData.map((member) => (
          <div key={member.id}>
            <Image src={member.image} alt={member.name} width={339} height={339} className='w-full h-auto rounded-2xl mb-6' />
            <h5>{member.name}</h5>
            <p>{member.position}</p>
          </div>
        ))}
